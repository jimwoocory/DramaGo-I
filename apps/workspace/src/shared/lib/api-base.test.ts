...
describe("api-base", () => {
	...
	it("uses the dev server origin inside desktop dev", () => {
		...
		expect(apiURL("/projects")).toBe("http://localhost:8080/api/v1/projects");
		expect(apiURL("/api/v1/agent/events")).toBe("http://localhost:8080/api/v1/agent/events");
	});

	it("uses the packaged sidecar origin supplied by the trusted desktop bridge", () => {
		vi.stubEnv("DEV", false);
		enableDesktopRuntime("http://localhost:53124");

		expect(apiOrigin()).toBe("http://localhost:53124");
		expect(apiBaseURL()).toBe("http://localhost:53124/api/v1");
		expect(apiResourceURL("/api/media/assets/image-1/content")).toBe(
			"http://localhost:53124/api/v1/media-assets/image-1/content",
		);
		expect(apiResourceURL("/api/v1/media-assets/image-1/content")).toBe(
			"http://localhost:53124/api/v1/media-assets/image-1/content",
		);
		expect(apiResourceURL("api/v1/projects/project-1/assets/asset-1/content")).toBe(
			"http://localhost:53124/api/v1/projects/project-1/assets/asset-1/content",
		);
	...
	it("uses the configured local server port inside desktop runtime", () => {
		...
		expect(apiOrigin()).toBe("http://localhost:49152");
		expect(apiBaseURL()).toBe("http://localhost:49152/api/v1");
	});
});
