	...
	const result = renderHook(() => {
		...
		return {
			...
			...useGenerationSubmit({
				...
				shotManifestId: options.shotManifestId,
...
describe("useGenerationSubmit", () => {
	...
	it("submits a manifest-backed video without free-form prompt or stale editor references", async () => {
		vi.mocked(sendGenerationMessage).mockResolvedValue(generationResponse());
		const { result } = renderSubmitHook({
			prompt: "   ",
			shotManifestId: "shot-manifest-1",
