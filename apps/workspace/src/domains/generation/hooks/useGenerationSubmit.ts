...
export const useGenerationSubmit = ({
	...
	const submitGeneration = useCallback(
		async (overrides: GenerationSubmitOverrides = {}) => {
			const requestShotManifestId = (overrides.shotManifestId ?? shotManifestId)?.trim() ?? "";
			const promptInput = overrides.prompt ?? promptRef?.current ?? prompt;
			const nextPrompt = promptInput.trim();
			// A Shot Manifest is the authoritative execution contract. Do not let stale
			// editor/media-reference state block or decorate a manifest-backed request;
			// the server compiler supplies the final Canon reference set.
			const requestReferenceAssetIds = requestShotManifestId
				? []
				: (overrides.referenceAssetIds ?? effectiveReferenceAssetIds);
			const requestReferenceBindings = requestShotManifestId
				? []
				: (overrides.referenceBindings ?? effectiveReferenceBindings);
			const requestReferenceUrls = requestShotManifestId
				? []
				: (overrides.referenceUrls ?? effectiveReferenceUrls);
			...
			if (
				(!nextPrompt && !requestDocumentContext && !requestShotManifestId) ||
			...
			try {
				...
				const generationPayload: GenerationMessageRequest = {
					...
					documentId: requestDocumentId || undefined,
					sectionId: requestSectionId || undefined,
					shotManifestId: requestShotManifestId || undefined,
					documentContext: requestDocumentContext ?? undefined,
					capabilityId: requestTaskType,
					notificationTarget: requestNotificationTarget ?? undefined,
					routeId: requestRoute.id,
