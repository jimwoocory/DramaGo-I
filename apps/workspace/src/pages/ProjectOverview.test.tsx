...
describe("ProjectOverview", () => {
	beforeEach(() => {
		...
		vi.mocked(httpClient.post).mockImplementation(async (url, payload) => {
			if (String(url) === "/projects/project-a/canon/sync") {
				return apiResponse({
					referencesCreated: 0,
					referencesReused: 0,
					selectionsSkipped: 0,
					resources: { created: 0, updated: 0, locked: 0, skipped: 0 },
				});
			}
			if (String(url) === "/projects/project-a/shot-manifests/sync") {
				return apiResponse({
					created: 0,
					updated: 0,
					reused: shotManifestsFixture.length,
					skipped: 0,
				});
			}
		...
		vi.mocked(httpClient.get).mockImplementation(async (url, config) => {
			...
			if (requestUrl === "/projects/project-a/shot-manifests") {
				...
				return apiResponse(
					shotManifestsFixture.filter(
						(item) => !documentId || (item as { documentId?: string }).documentId === documentId,
					),
				);
			}
	...
	it("submits document-derived resources for background batch image generation", async () => {
		...
		await waitFor(() => {
			const batchCalls = vi
				...
				.mock.calls.filter(([url]) => url === "/generation/batches");
		...
		const [url, payload] = vi
			.mocked(httpClient.post)
			.mock.calls.find(([candidate]) => candidate === "/generation/batches")!;
	...
	it("opens Production Shot keyframe generation with the exact shotManifestId", async () => {
		shotManifestsFixture = [
			{
				actionText: "沈阁从黑暗水面坠入湖中，单一低角度入水镜头。",
				audioText: "音频：入水声",
		...
		fireEvent.click(screen.getByRole("button", { name: "第一章分镜脚本 视频生成" }));
		const dialog = await screen.findByRole("dialog", { name: "视频生成 · 第一章分镜脚本" });
		expect(within(dialog).getByText("制作镜头 · 关键帧")).toBeInTheDocument();
		expect(within(dialog).getByText("镜头 01")).toBeInTheDocument();
		expect(within(dialog).getByText("0.00–3.20s")).toBeInTheDocument();
		expect(within(dialog).getByText("兼容入口 · 分镜组视频")).toBeInTheDocument();

		fireEvent.click(within(dialog).getByRole("button", { name: "生成关键帧" }));
		await waitFor(() => {
			expect(dialogMocks.MediaGenerationDialog.mock.calls.at(-1)?.[0].request).toMatchObject({
				kind: "image",
				projectId: "project-a",
				resolveLatestSection: false,
				shotManifestId: "shot-production-001",
				statusResourceKey: "shot-production-001",
				section: expect.objectContaining({
					blockId: "section_reel_01",
					documentId: "storyboard-a",
				}),
	...
	it("submits storyboard video reels for background batch video generation", async () => {
		...
		await waitFor(() => {
			const batchCalls = vi
				.mocked(httpClient.post)
				.mock.calls.filter(([candidate]) => candidate === "/generation/batches");
			expect(batchCalls).toHaveLength(1);
		});
		const [url, payload] = vi
			.mocked(httpClient.post)
