...
import {
	getProjectCanon,
	getProjectShotManifests,
	projectCanonKey,
	projectShotManifestsKey,
	syncProjectCanon,
	syncProjectShotManifests,
...
interface OverviewBatchGenerationReferenceContext {
	...
	legacyShotManifestsByDocumentSection: ReadonlyMap<string, ShotManifestRecord>;
}
...
export const ProjectOverview: React.FC = () => {
	...
	const storyboardDocumentIds = useMemo(
		() => (storyboardVideoResources?.groups ?? []).map((group) => group.documentId).filter(Boolean),
		[storyboardVideoResources?.groups],
	);
	const { data: canonAssets = [], mutate: mutateCanonAssets } = useSWR(
		projectId ? projectCanonKey(projectId) : null,
		() => getProjectCanon(projectId ?? ""),
	);
	const { data: shotManifests = [], mutate: mutateShotManifests } = useSWR(
		projectId && storyboardDocumentIds.length > 0
			? projectShotManifestsKey(projectId, storyboardDocumentIds)
			: null,
	...
	useEffect(() => {
		if (!projectId || !workspaceDocumentResources || !storyboardVideoResources) return;
		if (continuitySyncedProjectRef.current === projectId) return;
		continuitySyncedProjectRef.current = projectId;
		let cancelled = false;
		void (async () => {
			try {
				await syncProjectCanon(projectId);
				await syncProjectShotManifests(projectId);
				if (cancelled) return;
				await Promise.all([
					mutateCanonAssets(),
					mutateShotManifests(),
		...
		mutateStoryboardVideoResources,
		projectId,
		storyboardVideoResources,
		toast,
		workspaceDocumentResources,
	]);

	const documentResources = workspaceDocumentResources?.resources ?? [];
	const storyboardVideoGroups = storyboardVideoResources?.groups ?? [];
	const legacyShotManifestByDocumentSection = useMemo(
		() =>
			new Map(
				shotManifests
					.filter((shot) => !shot.shotKey?.trim())
					.map((shot) => [`${shot.documentId}\u0000${shot.sectionId}`, shot] as const),
			),
		[shotManifests],
	);
	const productionShotManifests = useMemo(
		() => shotManifests.filter((shot) => Boolean(shot.shotKey?.trim())),
		[shotManifests],
	);
	const continuitySummary = useMemo(
		() => projectContinuitySummary(canonAssets, shotManifests),
		[canonAssets, shotManifests],
	);
	...
	const openProductionShotImageGeneration = useCallback(
		(group: WorkspaceStoryboardVideoDocumentGroup, shot: ShotManifestRecord) => {
			const reel = group.reels.find(
				(candidate) => (candidate.sectionId || candidate.blockId) === shot.sectionId,
			);
			if (!reel) {
				toast.error("制作镜头无法定位原分镜组", { description: shot.shotKey || shot.id });
				return;
			}
			setMediaGenerationRequest({
				kind: "image",
				...
				shotManifestId: shot.id,
				statusResourceKey: shot.id,
			});
		},
		[projectId, toast],
	);
	const openStoryboardVideoGeneration = useCallback(
		(group: WorkspaceStoryboardVideoDocumentGroup, reel: WorkspaceStoryboardVideoReel) => {
			const sectionId = reel.sectionId || reel.blockId;
			const shotManifest = legacyShotManifestByDocumentSection.get(
				`${group.documentId}\u0000${sectionId}`,
			);
			setMediaGenerationRequest({
				kind: "video",
				projectId: projectId ?? undefined,
				section: storyboardReelToSectionContext(group, reel),
				resolveLatestSection: false,
				shotManifestId: shotManifest?.id,
				statusResourceKey: reel.id,
			});
		},
		[legacyShotManifestByDocumentSection, projectId],
	);
	...
	const confirmBatchGeneration = useCallback(
		...
		[
			...
			projectId,
			legacyShotManifestByDocumentSection,
			selectedGenerationAssets,
			toast,
		],
	);
					...
					<main className="mx-auto flex min-h-full w-full max-w-5xl flex-col gap-4 px-4 py-4">
						...
						{config && !isLoading ? (
							<section className="grid gap-4">
								...
								/>
								<ProjectContinuitySummary summary={continuitySummary} />
								...
								<StoryboardVideoResourcesDialog
									...
									onGenerateProductionShot={openProductionShotImageGeneration}
									onPrepareWorkbench={prepareStoryboardWorkbench}
									productionShots={productionShotManifests}
									onOpenChange={(open) => {
...
interface ProjectContinuitySummaryValue {
	characterCores: number;
	sceneCores: number;
	propCores: number;
	variants: number;
	locked: number;
	shots: number;
	inheritedShots: number;
	compiledShots: number;
	conflicts: number;
}

const projectContinuitySummary = (
	canonAssets: readonly CanonAssetRecord[],
	shotManifests: readonly ShotManifestRecord[],
): ProjectContinuitySummaryValue => {
	const cores = canonAssets.filter((asset) => !asset.parentId);
	const productionShots = shotManifests.filter((shot) => Boolean(shot.shotKey?.trim()));
	const activeShots = productionShots.length > 0 ? productionShots : shotManifests;
	return {
		characterCores: cores.filter((asset) => asset.resourceType === "character").length,
		sceneCores: cores.filter((asset) => asset.resourceType === "scene").length,
		propCores: cores.filter((asset) => asset.resourceType === "prop").length,
		variants: canonAssets.length - cores.length,
		locked: canonAssets.filter((asset) => asset.status === "locked").length,
		shots: activeShots.length,
		inheritedShots: activeShots.filter((shot) => Boolean(shot.inheritsFromId)).length,
...
const ProjectContinuitySummary: React.FC<{ summary: ProjectContinuitySummaryValue }> = ({
	...
	const stats = [
		["角色 Core", summary.characterCores],
		["场景 Core", summary.sceneCores],
		["道具 Core", summary.propCores],
		["造型/状态 Variant", summary.variants],
		["结构化 Shot", summary.shots],
		["继承上一镜", summary.inheritedShots],
	] as const;
	return (
		<section className="bg-card">
			<div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
				<div className="flex min-w-0 items-center gap-2">
					...
					<div className="min-w-0">
						<h2 className="text-sm font-semibold text-foreground">一致性资产</h2>
						<p className="text-xs text-muted-foreground">
							角色身份、造型、场景、道具与镜头状态由系统自动绑定和继承。
						</p>
					</div>
				</div>
				<div className="flex flex-wrap items-center gap-1.5">
					{summary.locked > 0 ? <Badge variant="secondary">锁定 {summary.locked}</Badge> : null}
					{summary.compiledShots > 0 ? (
						<Badge variant="secondary">已编译 {summary.compiledShots}</Badge>
					) : null}
					{summary.conflicts > 0 ? (
						<Badge variant="destructive">冲突 {summary.conflicts}</Badge>
					) : null}
				</div>
			</div>
			<div className="mt-3 grid gap-2 sm:grid-cols-3 xl:grid-cols-6">
				{stats.map(([label, value]) => (
					<div key={label} className="rounded-sm border border-border bg-ide-editor px-3 py-3">
...
const productionShotTimeLabel = (shot: ShotManifestRecord) => {
	const duration = Number.isFinite(shot.durationSeconds)
		? Math.max(0, shot.durationSeconds ?? 0)
		: 0;
	const end = Number.isFinite(shot.endSeconds) ? Math.max(0, shot.endSeconds ?? 0) : duration;
	const start = Number.isFinite(shot.startSeconds)
		? Math.max(0, shot.startSeconds ?? 0)
		: Math.max(0, end - duration);
...
const StoryboardVideoResourcesDialog: React.FC<{
	...
	onGenerateProductionShot: (
		group: WorkspaceStoryboardVideoDocumentGroup,
		shot: ShotManifestRecord,
	) => void;
	onOpenChange: (open: boolean) => void;
	onPrepareWorkbench: (group: WorkspaceStoryboardVideoDocumentGroup) => void;
	productionShots: ShotManifestRecord[];
}> = ({
	error,
	generationStatuses,
	group,
	...
	const visibleProductionShots = useMemo(
		() =>
			group
				? productionShots
						.filter((shot) => shot.documentId === group.documentId && Boolean(shot.shotKey?.trim()))
						.sort(
							(first, second) =>
								first.sequence - second.sequence || first.id.localeCompare(second.id),
						)
				: [],
		[group, productionShots],
	);
			...
			>
				<div className="flex h-full min-h-0 flex-col bg-ide-editor">
					<TabsContent value="list" className="m-0 flex h-full min-h-0 flex-col">
						...
						{!isLoading && !error && group.reels.length > 0 ? (
							<>
								{visibleProductionShots.length > 0 ? (
									<section className="border-b border-border bg-card px-4 py-4">
										<div className="mb-3 flex flex-wrap items-center justify-between gap-2">
											<div>
												<h3 className="text-sm font-semibold text-foreground">制作镜头 · 关键帧</h3>
												<p className="mt-0.5 text-xs text-muted-foreground">
													每个时间段独立生成一张单视觉意图关键帧，自动使用对应角色、场景、道具与上一镜引用。
												</p>
											</div>
											<Badge variant="secondary">{visibleProductionShots.length} 镜头</Badge>
										</div>
										<div className="grid max-h-72 gap-2 overflow-y-auto pr-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
											{visibleProductionShots.map((shot) => (
												<article
													key={shot.id}
													className="flex min-w-0 flex-col gap-2 rounded-sm border border-border bg-ide-editor p-3"
												>
													...
													<Button
														...
														onClick={() => onGenerateProductionShot(group, shot)}
													>
...
const overviewBatchGenerationItems = (
	...
	return dialog.reels.map((reel) => {
		const section = storyboardReelToSectionContext(dialog.group, reel);
		const shotManifest = referenceContext.legacyShotManifestsByDocumentSection.get(
			`${dialog.group.documentId}\u0000${reel.sectionId || reel.blockId}`,
		);
		return {
			id: reel.id,
			request: generationBatchRequestForSection({
...
const generationBatchRequestForSection = ({
	...
	projectId,
	prompt,
	promptSupplements: shotManifestId ? [] : settings.promptSupplements,
	promptOptimization: shotManifestId ? undefined : settings.promptOptimization,
	provider: settings.route.provider,
	...(shotManifestId
		? { referenceAssetIds: [], referenceBindings: [], referenceUrls: [] }
		: batchGenerationReferences(settings, section, prompt, kind, referenceContext)),
	resourceType,
	shotManifestId: shotManifestId?.trim() || undefined,
