...
export const useMarkdownEditorPerformance = ({
	...
	const initialSerializedMarkdownRef = useRef<string | null>(null);
	...
	const captureInitialSerializedMarkdown = useCallback((editor: Editor) => {
		if (editor.isDestroyed) {
			initialSerializedMarkdownRef.current = null;
			return;
		}
		const markdown = editor.getMarkdown();
		initialSerializedMarkdownRef.current =
			markdown === emittedMarkdownRef.current ? null : markdown;
	}, []);
	...
	const flushPendingMarkdownChange = useCallback(() => {
		...
		const initialSerializedMarkdown = initialSerializedMarkdownRef.current;
		if (initialSerializedMarkdown !== null) {
			initialSerializedMarkdownRef.current = null;
			if (markdown === initialSerializedMarkdown) return;
		}
	...
	return {
		captureInitialSerializedMarkdown,
		emittedMarkdownRef,
		flushPendingMarkdownChange,
