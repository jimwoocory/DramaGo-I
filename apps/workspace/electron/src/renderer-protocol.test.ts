...
describe("desktop renderer protocol", () => {
	...
	it("allows the advertised localhost sidecar without opening arbitrary remote scripts", () => {
		const policy = rendererContentSecurityPolicy("http://localhost:54321");
		expect(policy.split("; ")).toContain("script-src 'self'");
		expect(policy).toContain("connect-src 'self' http://localhost:54321 https: wss:");
		expect(policy).not.toContain("'unsafe-eval'");
	});

	it("still accepts a literal IPv4 loopback origin for compatibility", () => {
