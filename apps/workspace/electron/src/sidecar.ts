...
export const startServerSidecar = async (): Promise<SidecarConnection | null> => {
	...
	child = spawned;
	// Keep the server bound to 127.0.0.1, but advertise localhost to renderer/agent clients.
	// Some Windows VPN/TUN/transparent-proxy stacks treat literal loopback IPs differently
	// from the localhost hostname and may incorrectly intercept 127.0.0.1 traffic.
	connection = { origin: `http://localhost:${serverPort}`, token };
