...
func TestCharacterWriterUsesCoreAndNestedVisualVariants(t *testing.T) {
	bundle, err := Builtin(context.Background())
	if err != nil {
		t.Fatalf("Builtin() error = %v", err)
	}
	...
	for _, entry := range bundle.Entries {
		...
		for _, fragment := range []string{
			...
			if !strings.Contains(body, fragment) {
				t.Fatalf("character-writer missing Canon/Variant rule %q:\n%s", fragment, body)
			}
		}
		return
	}
...
func TestCreativeSkillsPreserveCanonFactsAndStoryboardUsesContinuityContract(t *testing.T) {
	bundle, err := Builtin(context.Background())
	if err != nil {
		t.Fatalf("Builtin() error = %v", err)
	}
	...
	for _, entry := range bundle.Entries {
		if entry.Kind == pack.KindSkill {
			bodies[entry.Slug] = entry.Description + "\n" + entry.Body
		}
	}

	for slug, fragments := range map[string][]string{
		"character-writer": {
			"通读锁事实",
			"可选视觉建议",
			"不能反向改变剧情",
		...
		"storyboard-writer": {
			...
			"状态变化：无（继承上一镜）",
			"不要在每个镜头重复写一遍",
			"原剧本没有的对白、动作、道具用途、时间跳转和因果关系不得新增",
			"不得把约 12 分钟内容无说明压成约 4 分钟",
		},
	} {
		body := bodies[slug]
		if body == "" {
		...
		for _, fragment := range fragments {
			if !strings.Contains(body, fragment) {
				t.Fatalf("%s missing contract %q:\n%s", slug, fragment, body)
			}
		}
	}

	storyboardBody := bodies["storyboard-writer"]
	for _, forbidden := range []string{
		...
		if strings.Contains(storyboardBody, forbidden) {
			t.Fatalf("storyboard-writer regressed to repeated global prompt rule %q:\n%s", forbidden, storyboardBody)
		}
	}
}
