...
func newAPIHandler(config Config) *apiHandler {
	...
	canonService := servicecanon.NewService(workspaceRepos.Canon, workspaceReposErr)
	canonService.SetDocumentResourceProvider(workspaceState.StateService().Documents)
	canonService.SetSelectedAssetProvider(workspaceRepos.GenerationTasks)
	shotManifestService := serviceshotmanifest.NewService(workspaceRepos.ShotManifests, canonService, workspaceReposErr)
...
func defaultAgentBridgeURL(host string, port int) string {
	host = advertisedLoopbackHost(host)
	if port <= 0 {
		port = 8080
	}
...
func advertisedLoopbackHost(host string) string {
...
func codexRelayBridgeBaseURL(agentBridgeURL string) string {
	...
	if host := advertisedLoopbackHost(parsed.Hostname()); host == "localhost" {
