...
func TestDefaultAgentBridgeURL(t *testing.T) {
	tests := []struct {
		...
		{name: "wildcard", host: "0.0.0.0", port: 19090, want: "http://localhost:19090"},
		{name: "ipv4 loopback", host: "127.0.0.1", port: 19090, want: "http://localhost:19090"},
		{name: "ipv6 loopback", host: "::1", port: 19090, want: "http://localhost:19090"},
		{name: "explicit localhost", host: "localhost", port: 19090, want: "http://localhost:19090"},
		{name: "explicit lan host", host: "192.168.1.10", port: 19090, want: "http://192.168.1.10:19090"},
	}
...
func TestCodexRelayBridgeBaseURL(t *testing.T) {
	tests := []struct {
		...
		{
			name: "spawn endpoint ipv4 loopback",
			url:  "http://127.0.0.1:8080/api/v1/internal/agent/spawn",
			want: "http://localhost:8080",
		},
		{
			name: "already base url ipv4 loopback",
