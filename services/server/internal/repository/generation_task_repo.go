...
// LatestCompletedImageAssetIDBySection returns the newest surviving image output for one storyboard section.
// It intentionally returns only a physical asset ID; callers decide whether it is suitable as a short-lived continuity reference.
func (repo *GenerationTaskRepository) LatestCompletedImageAssetIDBySection(projectID string, documentID string, sectionID string) (string, bool, error) {
	projectID = domain.CleanProjectID(projectID)
	...
	err := repo.db.
		Table("generation_task_assets").
		Select("generation_task_assets.asset_id AS asset_id").
		Joins("JOIN generation_tasks ON generation_tasks.id = generation_task_assets.task_id").
		Joins("JOIN assets ON assets.id = generation_task_assets.asset_id").
		Where("generation_tasks.project_id = ?", projectID).
		Where("generation_tasks.document_id = ?", documentID).
		Where("generation_tasks.section_id = ?", sectionID).
		Where("generation_tasks.kind = ?", "image").
		Where("LOWER(TRIM(generation_tasks.status)) IN ?", []string{"completed", "succeeded", "success"}).
		Where("assets.project_id = ?", projectID).
		Order("generation_tasks.created_at DESC").
		Order("generation_tasks.updated_at DESC").
		Order("generation_task_assets.slot_index ASC").
		Limit(1).
		Scan(&result).Error
	if err != nil {
		return "", false, fmt.Errorf("finding latest completed image for section: %w", err)
	}
	result.AssetID = strings.TrimSpace(result.AssetID)
	return result.AssetID, result.AssetID != "", nil
}

// LatestCompletedReferencedImageAssetIDBySection returns the newest surviving image output
// for one storyboard section whose generation task actually carried image reference rows.
// This prevents pre-Canon legacy renders with no generation_task_references from being
// promoted into previous-shot continuity anchors while preserving continuity for current
// referenced runs. source_refs_json is content provenance and is intentionally not used here.
func (repo *GenerationTaskRepository) LatestCompletedReferencedImageAssetIDBySection(projectID string, documentID string, sectionID string) (string, bool, error) {
	projectID = domain.CleanProjectID(projectID)
	documentID = strings.TrimSpace(documentID)
	sectionID = strings.TrimSpace(sectionID)
	if projectID == "" || documentID == "" || sectionID == "" {
	...
	if err := repo.db.
		...
		Where("LOWER(TRIM(generation_tasks.status)) IN ?", []string{"completed", "succeeded", "success"}).
		Where("assets.project_id = ?", projectID).
		Where("EXISTS (SELECT 1 FROM generation_task_references WHERE generation_task_references.task_id = generation_tasks.id AND (COALESCE(TRIM(generation_task_references.asset_id), '') <> '' OR COALESCE(TRIM(generation_task_references.url), '') <> ''))").
		Order("generation_tasks.created_at DESC").
		Order("generation_tasks.updated_at DESC").
		Order("generation_task_assets.slot_index ASC").
		Limit(50).
		Scan(&rows).Error; err != nil {
		return "", false, fmt.Errorf("finding latest referenced image for section: %w", err)
	}
...
// LatestCompletedReferencedImageAssetIDByShotManifest returns the newest surviving image
// output generated for one exact Shot Manifest. Production Shots may share the same
// document/section with sibling beats, so continuity must not fall back to section-level
// selection once a non-empty shot_manifest_id exists.
func (repo *GenerationTaskRepository) LatestCompletedReferencedImageAssetIDByShotManifest(projectID string, shotManifestID string) (string, bool, error) {
	projectID = domain.CleanProjectID(projectID)
	shotManifestID = strings.TrimSpace(shotManifestID)
	if projectID == "" || shotManifestID == "" {
		return "", false, nil
	}
	type row struct {
		AssetID string `gorm:"column:asset_id"`
	...
	if err := repo.db.
		...
		Joins("JOIN assets ON assets.id = generation_task_assets.asset_id").
		Where("generation_tasks.project_id = ?", projectID).
		Where("generation_tasks.shot_manifest_id = ?", shotManifestID).
		Where("generation_tasks.kind = ?", "image").
		Where("LOWER(TRIM(generation_tasks.status)) IN ?", []string{"completed", "succeeded", "success"}).
		Where("assets.project_id = ?", projectID).
		Where("EXISTS (SELECT 1 FROM generation_task_references WHERE generation_task_references.task_id = generation_tasks.id AND (COALESCE(TRIM(generation_task_references.asset_id), '') <> '' OR COALESCE(TRIM(generation_task_references.url), '') <> ''))").
		...
		Limit(50).
		Scan(&rows).Error; err != nil {
		return "", false, fmt.Errorf("finding latest referenced image for shot manifest: %w", err)
	}
	for _, candidate := range rows {
		assetID := strings.TrimSpace(candidate.AssetID)
		if assetID != "" {
