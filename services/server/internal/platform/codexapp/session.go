...
func (buffer *boundedTextBuffer) Write(value []byte) (int, error) {
	if buffer == nil {
...
func (buffer *boundedTextBuffer) String() string {
	if buffer == nil {
		return ""
	}
	buffer.mu.Lock()
...
func start(parent context.Context, binPath string, overrides []string) (*Session, error) {
	...
	session := &Session{cancel: cancel, cmd: cmd, stdin: stdin, scan: newMessageScanner(stdout), stderr: stderr}
...
func safeAppServerStderr(value string) string {
	value = strings.TrimSpace(value)
	if value == "" {
		return ""
	}
	...
	for _, marker := range []string{"authorization:", "bearer ", "access_token", "refresh_token"} {
...
func (session *Session) read(ctx context.Context) (Message, error) {
	...
	if !session.scan.Scan() {
		...
		if detail := safeAppServerStderr(session.stderr.String()); detail != "" {
			return Message{}, fmt.Errorf("app-server closed stdout: %s", detail)
		}
