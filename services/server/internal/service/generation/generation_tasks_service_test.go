...
func TestStaleUnpollableJimengImageIsRecoveredAsRetryableTimeout(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "workspace.db")
	service := newTestGenerationTaskService(t, dbPath, nil)
	for _, taskID := range []string{"jimeng-stale", "jimeng-fresh"} {
		if err := service.Upsert(GenerationTaskRecord{
			ID: taskID, Kind: "image", RouteID: "jimeng.seedream-4.7", FamilyID: "seedream",
			VersionID: "seedream-4.7", Provider: "jimeng", ModelID: "jimeng.seedream-4.7", Model: "4.7",
			Prompt: "golden shot", Status: "running",
		}); err != nil {
			t.Fatalf("Upsert(%s) error = %v", taskID, err)
		}
	}
	...
	staleAt := time.Now().UTC().Add(-jimengImageGenerationHardTimeout - time.Second)
	if err := db.Model(&domain.GenerationTaskModel{}).Where("id = ?", "jimeng-stale").Update("updated_at", staleAt).Error; err != nil {
		t.Fatalf("aging stale task: %v", err)
	}

	pending, err := service.ListPending(10)
	if err != nil {
		t.Fatalf("ListPending() error = %v", err)
	}
	if len(pending) != 1 || pending[0].ID != "jimeng-stale" {
		t.Fatalf("pending = %+v, want only stale orphan", pending)
	}

	workflow := NewGenerationService(nil, service, nil)
	workflow.PollGenerationTask(context.Background(), pending[0])
	recovered, ok, err := service.Get("jimeng-stale")
	if err != nil || !ok {
		t.Fatalf("Get(recovered) ok=%v error=%v", ok, err)
	}
	if recovered.Status != "failed" || recovered.ErrorCode != "timeout" || !recovered.Retryable {
		t.Fatalf("recovered = %+v, want retryable timeout failure", recovered)
	}
	fresh, ok, err := service.Get("jimeng-fresh")
	if err != nil || !ok || fresh.Status != "running" {
		t.Fatalf("fresh = %+v ok=%v err=%v, want running untouched", fresh, ok, err)
	}
}
