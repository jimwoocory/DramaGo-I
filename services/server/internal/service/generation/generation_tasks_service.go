...
// LatestImageAssetIDForSection exposes the newest successful image output for short-lived shot continuity.
func (service *GenerationTaskService) LatestImageAssetIDForSection(projectID string, documentID string, sectionID string) (string, bool, error) {
	if service == nil {
		return "", false, errors.New("generation task service is nil")
	}
	if service.initErr != nil {
	...
	return service.repo.LatestCompletedReferencedImageAssetIDBySection(projectID, documentID, sectionID)
}

// LatestImageAssetIDForShotManifest exposes the newest successful image generated for
// one exact Shot Manifest. Production Shot continuity uses this instead of section-level
// lookup because multiple beats intentionally share one storyboard H2 section.
func (service *GenerationTaskService) LatestImageAssetIDForShotManifest(projectID string, shotManifestID string) (string, bool, error) {
	if service == nil {
		return "", false, errors.New("generation task service is nil")
	}
	if service.initErr != nil {
	...
	service.mu.RLock()
	defer service.mu.RUnlock()
	return service.repo.LatestCompletedReferencedImageAssetIDByShotManifest(projectID, shotManifestID)
}
...
func (service *GenerationTaskService) ListPending(limit int) ([]GenerationTaskRecord, error) {
	...
	// Image tasks normally enter polling only after inline generation hands them off as
...
func (service *GenerationTaskService) applyDefaultSelectedAssetLocked(task *GenerationTaskRecord) error {
	...
	clearModel := domain.ProjectSelectedAssetModel{
		...
		ResourceID:       domain.StringPtr(generationTaskSelectionResourceID(*task)),
...
func generationTaskRecordFromModel(model generationTaskModel) (GenerationTaskRecord, error) {
	task := GenerationTaskRecord{
		...
		ShotManifestID: domain.StringValue(model.ShotManifestID),
...
func selectedAssetResourceFromTask(task GenerationTaskRecord) (string, string) {
	...
	resourceID := generationTaskSelectionResourceID(task)
...
func generationTaskSelectionResourceID(task GenerationTaskRecord) string {
	if strings.TrimSpace(task.ResourceType) == "storyboard" {
		if shotManifestID := strings.TrimSpace(task.ShotManifestID); shotManifestID != "" {
			return shotManifestID
		}
	}
	return strings.TrimSpace(task.SectionID)
}
...
func isExpiredUnpollableJimengImage(task GenerationTaskRecord, now time.Time) bool {
	if !strings.EqualFold(strings.TrimSpace(task.Kind), string(coregeneration.KindImage)) ||
		!strings.EqualFold(strings.TrimSpace(task.Provider), coregeneration.ProviderJimeng) ||
		!strings.EqualFold(strings.TrimSpace(task.Status), "running") ||
	...
	return now.Sub(updatedAt) >= jimengImageGenerationHardTimeout
}
