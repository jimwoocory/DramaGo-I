...
func TestGenerationRequestTimeoutForUsesShorterJimengImageGuard(t *testing.T) {
	jimengImage := coregeneration.Request{Provider: coregeneration.ProviderJimeng, Kind: coregeneration.KindImage}
	if got := generationRequestTimeoutFor(jimengImage); got != 240*time.Second {
		t.Fatalf("jimeng image timeout = %s, want 240s", got)
	}
	jimengVideo := coregeneration.Request{Provider: coregeneration.ProviderJimeng, Kind: coregeneration.KindVideo}
	if got := generationRequestTimeoutFor(jimengVideo); got != generationRequestTimeout {
		t.Fatalf("jimeng video timeout = %s, want %s", got, generationRequestTimeout)
	}
	otherImage := coregeneration.Request{Provider: coregeneration.ProviderDMX, Kind: coregeneration.KindImage}
	if got := generationRequestTimeoutFor(otherImage); got != generationRequestTimeout {
		t.Fatalf("other image timeout = %s, want %s", got, generationRequestTimeout)
	}
}
