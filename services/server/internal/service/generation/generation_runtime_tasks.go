...
func (workflow *GenerationService) PollGenerationTask(ctx context.Context, task generationTaskRecord) {
	if isExpiredUnpollableJimengImage(task, time.Now().UTC()) {
		workflow.failUnpollableJimengImageTask(task)
		return
	}
...
func (workflow *GenerationService) submitPendingGeneration(
	...
	generationCtx, cancel := context.WithTimeout(ctx, generationRequestTimeoutFor(request))
	defer cancel()
...
func (workflow *GenerationService) failUnpollableJimengImageTask(task generationTaskRecord) {
	if workflow == nil || workflow.generationTasks == nil {
		return
	}
	timeoutErr := jimengImageExecutionTimeoutError()
	messageResponse := FailedGenerationResponse(task.ID, timeoutErr)
	failedTask := GenerationTaskWithMessage(task, messageResponse)
	existed, saveErr := workflow.generationTasks.UpsertExisting(failedTask)
	if saveErr != nil {
		_ = workflow.generationTasks.RecordAttempt(task.ID, "recover", task.Status, "即梦图片孤儿任务恢复失败。", saveErr)
		return
	}
	if !existed {
		return
	...
	workflow.syncGenerationNotificationTask(failedTask)
	_ = workflow.generationTasks.RecordAttempt(task.ID, "recover", messageResponse.Status, messageResponse.Message, timeoutErr)
}

func jimengImageExecutionTimeoutError() error {
	return fmt.Errorf(
		"即梦图片生成超时：本地执行超过 %d 秒仍未返回供应商任务结果，请重试。",
		int(jimengImageGenerationHardTimeout/time.Second),
	)
}
