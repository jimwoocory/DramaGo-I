...
const (
	generationRequestTimeout         = 1000 * time.Second
	jimengImageGenerationHardTimeout = 240 * time.Second
)

func generationRequestTimeoutFor(request coregeneration.Request) time.Duration {
	if strings.EqualFold(strings.TrimSpace(request.Provider), coregeneration.ProviderJimeng) && request.Kind == coregeneration.KindImage {
		return jimengImageGenerationHardTimeout
	}
	return generationRequestTimeout
}

type shotManifestCompiler interface {
	CompileAndPersist(projectID string, id string) (shotmanifest.CompileResult, error)
}
...
type GenerationService struct {
	...
	shotManifestCompiler          shotManifestCompiler
...
// SetShotManifestCompiler wires the structured Shot Manifest execution contract into generation.
func (workflow *GenerationService) SetShotManifestCompiler(compiler shotManifestCompiler) {
	workflow.shotManifestCompiler = compiler
}
...
func (workflow *GenerationService) CreateGenerationMessage(ctx context.Context, payload generationMessageRequest) (generationMessageResponse, int, error) {
	...
	if payload.ProjectID == "" {
		payload.ProjectID = GenerationProjectIDFromScopeID(payload.ScopeID)
	}
	if status, err := workflow.applyShotManifestCompilation(&payload); err != nil {
	...
	runCtx, cancel := context.WithTimeout(ctx, generationRequestTimeoutFor(generationRequest))
	defer cancel()
