...
func cleanupDeprecatedWorkspaceProjects(workspaceDir string, repos WorkspaceStateRepositories) error {
	...
	if err != nil {
		return fmt.Errorf("rebasing workspace-relative project storage: %w", err)
	}
	if rebased > 0 {
		slog.Info("workspace-relative project storage locations rebased", "count", rebased)
	}
...
func rebaseWorkspaceRelativeProjectDirs(workspaceDir string, repos WorkspaceStateRepositories) (int, error) {
	if repos.Workspace == nil {
		return 0, nil
	}
	...
	workspaceRoot := shared.ResolveWorkspaceDir(workspaceDir)
	rebased := 0
	for _, project := range projects {
		...
		relativePath := filepath.Clean(filepath.FromSlash(relativeDir))
		if relativePath == "." || filepath.IsAbs(relativePath) || relativePath == ".." || strings.HasPrefix(relativePath, ".."+string(filepath.Separator)) {
			// Absolute relativeDir values represent user-selected external projects.
			// Parent traversal is never valid for a workspace-owned project.
			continue
		}
		candidate := filepath.Clean(filepath.Join(workspaceRoot, relativePath))
		within, err := filepath.Rel(workspaceRoot, candidate)
		if err != nil || within == "." || within == ".." || strings.HasPrefix(within, ".."+string(filepath.Separator)) || filepath.IsAbs(within) {
			continue
		}
		info, err := os.Stat(candidate)
		if err != nil || !info.IsDir() {
			// Do not rewrite a record unless the relocated workspace actually contains
			// the project directory. This preserves external/missing project semantics.
			continue
		}
		currentDir := strings.TrimSpace(project.ProjectDir)
		if currentDir != "" {
			currentDir = shared.ResolveWorkspaceDir(currentDir)
		}
		if sameWorkspacePath(currentDir, candidate) {
			continue
		}
		updated, err := repos.Workspace.UpdateProjectStorageLocation(
			project.ID,
