...
const maxStoryboardReferenceIndexItems = 36
...
func buildReferenceIndexItems(request AgentRunRequest) []referenceIndexItem {
	...
	if shouldCompactStoryboardReferenceIndex(request, len(items)) {
		return compactStoryboardReferenceIndexItems(request, items)
	}
	return items
}

func shouldCompactStoryboardReferenceIndex(request AgentRunRequest, itemCount int) bool {
	if itemCount <= maxStoryboardReferenceIndexItems {
		return false
	}
	if request.Document != nil && strings.EqualFold(strings.TrimSpace(request.Document.Category), "storyboard") {
		return true
	}
...
type rankedReferenceIndexItem struct {
	item  referenceIndexItem
	index int
	score int
}

func compactStoryboardReferenceIndexItems(request AgentRunRequest, items []referenceIndexItem) []referenceIndexItem {
	if len(items) <= maxStoryboardReferenceIndexItems {
		return items
	}
	contextText := storyboardReferenceRankingText(request)
	ranked := make([]rankedReferenceIndexItem, 0, len(items))
	explicitCount := 0
	for index, item := range items {
		score := referenceIndexRelevanceScore(contextText, item)
		if item.Explicit {
			explicitCount++
			score += 1_000_000
		}
		ranked = append(ranked, rankedReferenceIndexItem{item: item, index: index, score: score})
	}
	sort.SliceStable(ranked, func(first, second int) bool {
		...
		return ranked[first].index < ranked[second].index
	})

	limit := maxStoryboardReferenceIndexItems
	if explicitCount > limit {
		limit = explicitCount
	}
	if limit > maxReferenceIndexItems {
		limit = maxReferenceIndexItems
	}
	if limit > len(ranked) {
		limit = len(ranked)
	}
	selected := ranked[:limit]
	// Preserve the original workspace order after choosing the relevant subset so
	// the rendered index remains stable and easy for the model to scan.
	sort.SliceStable(selected, func(first, second int) bool {
		return selected[first].index < selected[second].index
	})
	result := make([]referenceIndexItem, 0, len(selected))
	for _, rankedItem := range selected {
		result = append(result, rankedItem.item)
	}
	return result
}

func storyboardReferenceRankingText(request AgentRunRequest) string {
	parts := []string{strings.TrimSpace(request.Prompt)}
	if request.Document != nil && strings.EqualFold(strings.TrimSpace(request.Document.Category), "storyboard") {
		parts = append(parts, request.Document.Title, request.Document.Content)
	}
	return strings.ToLower(strings.Join(parts, "\n"))
}

func referenceIndexRelevanceScore(contextText string, item referenceIndexItem) int {
	if contextText == "" {
		return 0
	}
	score := 0
	title := strings.ToLower(strings.TrimSpace(item.Title))
	if title != "" && strings.Contains(contextText, title) {
		score += 1000 + len([]rune(title))
	}
	documentTitle := strings.ToLower(strings.TrimSpace(item.DocumentTitle))
	if documentTitle != "" && documentTitle != title && strings.Contains(contextText, documentTitle) {
		score += 100
	}
	return score
}
